...
ds_power = Gauge(
    ...
    "DS power (dBmV)",
    ["channel_id", "docsis_version"],
    registry=registry,
)
ds_snr = Gauge(
    "docsis_downstream_snr_db",
    "DS SNR (dB)",
    ["channel_id", "docsis_version"],
    registry=registry,
)
ds_uncorr = Gauge(
    "docsis_downstream_uncorrectables",
    "DS uncorrectables",
    ["channel_id"],
    registry=registry,
)
ds_corr = Gauge(
    "docsis_downstream_corrected",
    "DS corrected",
    ["channel_id"],
    registry=registry,
)
ds_locked = Gauge(
    "docsis_downstream_locked",
    "DS locked (1=yes)",
    ["channel_id"],
    registry=registry,
)
us_power = Gauge(
    "docsis_upstream_power_dbmv",
    "US power (dBmV)",
    ["channel_id", "docsis_version"],
    registry=registry,
)
us_t3 = Gauge(
    "docsis_upstream_t3_timeouts",
    "US T3 timeouts",
    ["channel_id"],
    registry=registry,
)
us_t4 = Gauge(
    "docsis_upstream_t4_timeouts",
    "US T4 timeouts",
    ["channel_id"],
    registry=registry,
)
...
poll_errors = Counter(
    "docsis_poll_errors_total", "Failed polls", registry=registry
)
alert_total = Counter(
    "docsis_alerts_total", "Alerts fired", ["severity"], registry=registry
)
...
def update_metrics(snap) -> None:
    ...
    for ch in snap.downstream:
        ...
        ds_locked.labels(channel_id=cid).set(
            1
            if ch.lock_status and "locked" in ch.lock_status.lower()
            else 0
        )
