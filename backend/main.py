...
from database import DB_PATH, init_db, purge_old_data
from diagnostics import get_diagnostics, get_history
from metrics import get_metrics_output, update_metrics
...
async def lifespan(app: FastAPI):
    await init_db()
    _scheduler.add_job(
        run_poll_cycle, "interval", seconds=settings.POLL_INTERVAL,
        id="poll", replace_existing=True,
    )
    _scheduler.add_job(
        purge_old_data, "interval", hours=24,
...
def optional_auth(
    ...
    ok = secrets.compare_digest(
        creds.username, settings.BASIC_AUTH_USER
    ) and secrets.compare_digest(
        creds.password, settings.BASIC_AUTH_PASS or ""
    )
    if not ok:
        raise HTTPException(401, headers={"WWW-Authenticate": "Basic"})
    return True
            ...
            for ch in snap.downstream:
                await db.execute(
                    ...
                    (
                        ...
                        ch.snr_db, ch.modulation, ch.lock_status, ch.corrected,
                        ch.uncorrectables, ch.docsis_version,
                    ),
                )
...
async def health():
    return {
        ...
        "last_poll": (
            _latest_snapshot.polled_at.isoformat() if _latest_snapshot else None
        ),
    }
...
async def overview():
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        async with db.execute(
            "SELECT COUNT(*) FROM alerts"
            " WHERE resolved_at IS NULL AND severity='warn'"
        ) as cur:
...
async def event_logs(
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        async with db.execute(
            f"SELECT * FROM event_logs {where} ORDER BY ts DESC LIMIT ? OFFSET ?",  # noqa: S608
            params + [limit, offset],
        ) as cur:
...
async def alerts_api(open_only: bool = False):
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        async with db.execute(
            f"SELECT * FROM alerts {where} ORDER BY fired_at DESC LIMIT 500",  # noqa: S608
            (),
        ) as cur:
            rows = await cur.fetchall()
